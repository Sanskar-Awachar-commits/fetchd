...
Configuration is stored at `~/.config/fetchd/config.json` (`%USERPROFILE%\.config\fetchd\config.json` on Windows).

You can override the config location with `--config <path>` or the `FETCHD_CONFIG` environment variable.

On first run, `fetchd` automatically creates `~/.config/fetchd/config.json`:
